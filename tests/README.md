...
Скрипт создаёт `.venv` и ставит зависимости из `tests/requirements.txt`.

После установки `pytest` доступен через обёртку `bin/pytest` (рекомендуется добавить `bin/` в `PATH` — это делает `setup-tests.sh`).

## Запуск тестов

Запускайте из **корня репозитория** (`diploma/`), не из подпапок сервисов.
